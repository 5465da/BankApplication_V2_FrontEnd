...
export default function BodyContainer() {
  ...
  const handleSubmit = () => {
    var config = {
      ...
      data: JSON.stringify({ "payment_type": "Mobile bill", "card": state.mobilePayment.creditCard.card_id, "payment_amount": state.mobilePayment.amount })
    };
  ...
  const formFrom = (
    <div className="fromForm fromFormConfirmation">
      <h1>From</h1>
      <p>{state.mobilePayment.creditCard.card_type}</p>
      <div>
        <Button
          id="submitButton"
          variant="contained"
